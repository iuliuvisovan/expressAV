...
router.post("/mediaVideos/upload", function(request, response) {
  ...
  fs.writeFile(newImageFullName, base64Data, "base64", function(err) {
    if (err) console.log(err);
      ...
      console.log(
        "******** Video image created from base64 encoded string and saved as: " +
          newImageFullName
      );
    ...
    if (!err) {
      if (modelName == "repertoireItem") {
        var sortedDocs = docs.sort(function(a, b) {
          var textA = a.composer
            .toUpperCase()
            .split(" ")
            .splice(-1);
          var textB = b.composer
            .toUpperCase()
            .split(" ")
            .splice(-1);
  ...
  models.forEach(function(item) {
    mongoose.model(item).find({}, function(err, docs) {
      if (err) console.log(err);
        ...
        propertiesAdded++;
        jsonObj[item] = docs;
        if (propertiesAdded == models.length) writeToBackupFile(jsonObj);
      }
    });
  });
}
